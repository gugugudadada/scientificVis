...
class InteractiveVolumeRenderer:
    ...
    def load_and_preprocess_data(self):
        ...
        self.plane_size = max(physical_size[0], physical_size[1]) * 1.2  # 稍大一点
    ...
    def _update_rendering(self):
        ...
        center = self.volume_center.copy()
        forward = look_at - camera_pos
        forward /= np.linalg.norm(forward)
        ...
        # 平面四角基于 right 和 up 向量
        point1 = center - right * half_size   # 左
        point2 = center + up * half_size      # 上
    ...
    #     def on_interaction_start(obj, event):
    #         self.needs_update = True

    #     def on_idle(obj, event):
    #         if self.needs_update:
    #             self._update_rendering()
    #             self.needs_update = False

    #     self.interactor.AddObserver("StartInteractionEvent", on_interaction_start)
    #     self.interactor.AddObserver("EndInteractionEvent", on_interaction_start)  # 保险
    #     self.interactor.AddObserver("TimerEvent", on_idle)
